# transformer_bert.py  -- BERT-Style Transformer Blocks
# An alternative transformer implementation based on the BERT/GPT architecture
# (from Karpathy's nanoGPT). Used internally by the main transformer module.
...
class Transformer_BERT(nn.Module):
    def __init__(self, context_len, latent_dim=128, num_head=4, num_layer=4, dropout_rate=0.0,  
                 use_pos_embd_image=False, use_pos_embd_action=False, query_num=60,
    ...
    def forward(self, x, pos_embd_image=None, query_embed=None):
        if not self.use_pos_embd_image and not self.use_pos_embd_action: #everything learned - severe overfitting
            ...
            # print("query embed size:", query_embed.shape)
            # print("pos embd image size:", pos_embd_image.shape)
