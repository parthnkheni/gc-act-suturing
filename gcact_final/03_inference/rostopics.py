#!/usr/bin/env python
# rostopics.py  -- Camera & Robot State Subscriber
# Connects to the da Vinci robot's data streams via ROS (Robot Operating
# System). Subscribes to live camera feeds (left endoscope, PSM1 wrist cam,
# PSM2 wrist cam) and robot state (arm positions, jaw angles). The inference
# scripts read from this module to get the latest camera images and joint
# states at each timestep. Topic names may need to be edited to match the
# specific dVRK setup at JHU.
...
class ros_topics:
  ...
  def __init__(self):
    ...
    # endoscope imgs
    self.endo_cam_psm1_sub = rospy.Subscriber("/PSM1/endoscope_img/compressed", 
                                            CompressedImage, self.get_endo_cam_psm1)
    self.endo_cam_psm2_sub = rospy.Subscriber("/PSM2/endoscope_img/compressed", 
