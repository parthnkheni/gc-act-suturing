#!/bin/bash
# train_gcact.sh  -- Train GC-ACT Models (Our Novel Contribution)
# Fine-tunes the ACT v2 checkpoints with gesture conditioning  -- this is the
# novel GC-ACT architecture. Only trains needle throw and knot tying (needle
# pickup has no gesture labels and already performs well). Loads the v2
# checkpoint, adds the gesture embedding input, and trains for 1000 more
# epochs with a lower learning rate. Produces the final GC-ACT checkpoints
# that achieved 0.88mm (NT) and 0.72mm (KT) mean position error.
