#!/usr/bin/env python3
# offline_eval.py  -- Model Accuracy Evaluation
# Tests how accurate the trained models are WITHOUT needing the physical robot.
# Replays real surgical episodes from the SutureBot dataset: feeds the real
# camera images to the model, compares its predicted actions to what the human
# surgeon actually did, and reports the error. Key metrics:
#   - Position error (mm)  -- how far off the predicted arm position is
#   - Rotation error (degrees)  -- how far off the predicted orientation is
#   - Jaw accuracy (%)  -- whether the gripper is open/closed at the right time
#   - Drift (mm)  -- whether error grows over time within an episode
# GO/NO-GO threshold: <2mm position error = safe to try on real robot.
"""
...
def main():
    ...
    parser.add_argument('--image_encoder', type=str, default='resnet18',
                        ...
                        help='Backbone (resnet18=v1, efficientnet_b3=v2/GC-ACT)')
    ...
    for subtask_name in subtasks_to_eval:
        ...
        # Load model
        policy = ACTPolicyOffline(ckpt_path, subtask_name, device=args.device,
